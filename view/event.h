...
// by a character (Unicode code point). C_TEXT is accompanied by a character
// (ASCII), to represent C_A, C_B, and so on. Keyboard layout shift information
// is not available for C_TEXT, e.g. C_+ on most keyboards generates C_=
// instead. CLICK and UNCLICK are mouse button down and up events, with DRAG
// being a mouse movement in between. These come with coordinates. SCROLL events
// are generated by a mouse scroll wheel, or equivalent touchpad gesture and are
// accompanied by an amount. BLINK and SAVE are timer events.
